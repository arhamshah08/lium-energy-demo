...
export function DocumentVaultForm({ projectId }: { projectId: string }) {
  ...
  function handleFile(type: DocumentRecord['type'], files: FileList | null) {
    if (!files?.[0]) return
    setUploaded((prev) => ({ ...prev, [type]: files[0].name }))
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const missing = DOC_SLOTS.filter((s) => s.required && !uploaded[s.type])
    if (missing.length) { setError(`Required: ${missing.map((s) => s.label).join(', ')}`); return }

    setSubmitting(true)
    const documents = Object.entries(uploaded).map(([type, filename]) => ({
        ...
        <CardBody className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {DOC_SLOTS.map(({ type, label, icon, hint, required }) => (
              <label key={type} className="block group cursor-pointer">
                ...
                >
                  {uploaded[type] ? (
                    <>
                      ...
                      <p className="text-caption text-secondary font-bold truncate max-w-full px-2 text-center">
                        {uploaded[type]}
                      </p>
                    </>
                  ) : (
